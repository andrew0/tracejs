import { createDefaultConfig, CONTINUA_PER_FEATURE, NUM_FEATURES, } from './trace-param';
...
export default class TraceNet {
    ...
    actFeatures() {
        ...
        const fsum = util.zeros2D(CONTINUA_PER_FEATURE, this.config.fSlices);
        for (let c = 0; c < CONTINUA_PER_FEATURE; c++)
            for (let f = 0; f < NUM_FEATURES; f++)
                for (let fslice = 0; fslice < this.config.fSlices; fslice++)
        ...
        for (let c = 0; c < CONTINUA_PER_FEATURE; c++) {
            for (let f = 0; f < NUM_FEATURES; f++) {
                for (let fslice = 0; fslice < this.config.fSlices; fslice++) {
                    this.featNet[c * NUM_FEATURES + f][fslice] -= Math.max(0, ffi[c][fslice] -
