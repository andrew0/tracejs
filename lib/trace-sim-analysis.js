...
const discoverItemsToWatch = (config) => {
    const itemsToWatch = config.domain == TraceDomain.WORDS ? config.sim.config.lexicon.map(x => x.phon) : config.sim.phonemes.sorted().map(x => x.label);
    const datasets = exports.doSimAnalysis(Object.assign({}, config, { itemsToWatch }));
    return datasets
...
exports.doSimAnalysis = (config) => {
    ...
    const contentType = kValue ? TraceContentType.RESPONSE_PROBABILITIES : TraceContentType.ACTIVATIONS;
    const dataSetLength = sim.getStepsRun();
    if (dataSetLength == 0)
        return [];
    if (contentType == TraceContentType.RESPONSE_PROBABILITIES || contentType == TraceContentType.ACTIVATIONS) {
                                ...
                                for (let iiNDS = 0; iiNDS < numDataSets; iiNDS++) {
                                    ...
                                    for (let competAlign = 0; competAlign < numSlices; competAlign++) {
                                        if (domain == TraceDomain.WORDS) {
                                            ...
                                            const wordExtent = Math.floor(sim.config.deltaInput / sim.config.slicesPerPhon) * sim.config.lexicon[iNDS].phon.length;
                                            const competExtent = Math.floor(sim.config.deltaInput / sim.config.slicesPerPhon) * sim.config.lexicon[iiNDS].phon.length;
                        ...
                        case TraceCalculationType.FRAUENFELDER:
                            ...
                            for (let iII = 0; iII < itemIndices.length; iII++) {
                                for (let iiII = 0; iiII < itemIndices.length; iiII++) {
                                    for (let competAlign = 0; competAlign < numSlices; competAlign++) {
                                        if (domain == TraceDomain.WORDS) {
                                            const wordExtent = Math.floor(sim.config.deltaInput / sim.config.slicesPerPhon) * sim.config.lexicon[itemIndices[iII]].phon.length;
                                            const competExtent = Math.floor(sim.config.deltaInput / sim.config.slicesPerPhon) * sim.config.lexicon[itemIndices[iiII]].phon.length;
        ...
        for (let iII = 0; iII < itemIndices.length; iII++) {
            ...
            if (domain == TraceDomain.WORDS) {
                if (calculationType == TraceCalculationType.MAX_POSTHOC)
                    oneSeries = { label: `${sim.config.lexicon[itemIndices[iII]].phon} [${alignmentPostHoc[itemIndices[iII]]}]`, data: [] };
                else
                    oneSeries = { label: sim.config.lexicon[itemIndices[iII]].phon, data: [] };
                // if (calculationType == TraceCalculationType.MAX_POSTHOC)
                ...
                oneSeries = { label: sim.phonemes.sorted()[itemIndices[iII]].label, data: [] };
        ...
        if (competType == TraceCompetitionType.RAW) { // competIndex, raw, not a slope line
            for (let iDSL = 0; iDSL < dataSetLength; iDSL++) {
                // X is time step, y is data
        ...
        else if (competType == TraceCompetitionType.FIRST_DERIVATIVE) { // first derivative
        ...
        else if (competType == TraceCompetitionType.SECOND_DERIVATIVE) { // second derivative
    ...
    if (data.length > 0) {
        return [
            ...
            ...Array.from(Array(data[0].data.length), (_, cycle) => [
                cycle,
                ...data.map(x => x.data[cycle].y.toFixed(18))
            ].join('\t'))
        ].join('\n');
    }
    return '';
