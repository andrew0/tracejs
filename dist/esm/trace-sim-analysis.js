...
})(TraceContentType || (TraceContentType = {}));
/** how alignment works */
export var TraceCalculationType;
...
})(TraceCalculationType || (TraceCalculationType = {}));
/** how choice works */
export var TraceChoice;
(function (TraceChoice) {
    TraceChoice[TraceChoice["NORMAL"] = 0] = "NORMAL";
    TraceChoice[TraceChoice["FORCED"] = 1] = "FORCED";
})(TraceChoice || (TraceChoice = {}));
export var TraceCompetitionType;
...
})(TraceCompetitionType || (TraceCompetitionType = {}));
/**
...
const discoverItemsToWatch = (config) => {
    ...
    const datasets = doSimAnalysis(Object.assign(Object.assign({}, config), { itemsToWatch }));
    return datasets
 ...
 */
export const doSimAnalysis = (config) => {
                ...
                for (let iNDS = 0; iNDS < numDataSets; iNDS++) {
                    for (let iSlice = 0; iSlice < numSlices; iSlice++) {
                        ...
                        if (sim.config.freqNode.RDL_post_c && domain == TraceDomain.WORDS) {
                            ...
                            responseStrengthData[iDSL][iNDS][iSlice] = applyPostActivationScaling(sim.config.freqNode, sim.config.lexicon[iNDS].freq, responseStrengthData[iDSL][iNDS][iSlice]);
                        }
                    }
                }
            }
...
export const formatAnalysis = (data, padLabels = false) => {
    if (data.length > 0) {
        return [
