    ...
    actFeatures() {
        ...
        if (this.inputSlice < this.config.fSlices) {
            for (let fIndex = 0; fIndex < trace_param_1.CONTINUA_PER_FEATURE * trace_param_1.NUM_FEATURES; fIndex++) {
                for (let fslice = this.inputSlice + 1; fslice < this.config.fSlices && fslice < this.inputSlice + 1 + this.__nreps; fslice++) {
        ...
        for (let c = 0; c < trace_param_1.CONTINUA_PER_FEATURE; c++) {
            for (let f = 0; f < trace_param_1.NUM_FEATURES; f++) {
                for (let fslice = 0; fslice < this.config.fSlices; fslice++) {
                    this.featNet[c * trace_param_1.NUM_FEATURES + f][fslice] -= Math.max(0, ffi[c][fslice] -
                        Math.max(0, this.featLayer[c * trace_param_1.NUM_FEATURES + f][fslice] * this.config.gamma.F));
                }
            }
        }
    }
                            ...
                            if (this.phonLayer[phon][pslice] > 0)
                                ...
                                activation +=
                                    this.pfw[phon][cont][d] *
                                        ...
                                        this.phonemes.byIndex(phon).features[cont * trace_param_1.NUM_FEATURES + feat];
                        }
                    }
                ...
                if (this.config.freqNode.RDL_rest_s &&
                    ...
                    tt =
                        this.wordLayer[word][slice] -
                            (this.config.rest.W +
                                response_probability_1.applyRestScaling(this.config.freqNode, this.config.lexicon[word].freq)) +
                            (this.config.rest.W +
                                response_probability_1.applyRestScaling(this.config.primeNode, this.config.lexicon[word].prime));
