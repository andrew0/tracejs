...
class TracePhones {
    constructor(phonemes) {
        ...
        this.phonemes = phonemes.map((x) => (Object.assign({}, x)));
    ...
    byLabel(label) {
        return (this.phonemes.find((x) => x.label == label) ||
     ...
     */
    spreadPhons(spread, scale, min, max) {
        ...
        // spread offset
        let maxspread = 0;
        const computeSpreadOffset = (phons) => {
        ...
        const computeSpread = (phons) => {
            // loop over phonemes
            for (const phon of phons) {
                phon.spread = util.zeros2D(trace_param_1.NUM_FEATURES * trace_param_1.CONTINUA_PER_FEATURE, maxspread * 4);
                phon.norm = 0;
                // loop over continuum
                for (let cont = 0; cont < trace_param_1.NUM_FEATURES * trace_param_1.CONTINUA_PER_FEATURE; cont++) {
                    if (phon.features[cont] > 0) {
                        const spreadSteps = Math.floor(cont / trace_param_1.NUM_FEATURES);
                        // delta is the amount to ramp up/down
                        const delta = (phon.features[cont] * max - phon.features[cont] * min) /
     ...
     */
    makePhonemeContinuum(from, to, steps) {
...
//# sourceMappingURL=trace-phones.js.map